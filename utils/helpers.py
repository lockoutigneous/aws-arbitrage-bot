...
def append_to_file(file_path, content, add_newline=True):
    ...
    Args:
        file_path (str): Đường dẫn đến tệp tin
        content (str): Nội dung cần thêm
        add_newline (bool): Có thêm dòng mới trước nội dung không
        
    Returns:
        bool: True nếu thành công, False nếu thất bại
        
    Raises:
        IOError: Nếu không thể ghi vào tệp tin
    """
    try:
        ...
        print(f"Lỗi I/O khi ghi vào tệp tin {file_path}: {e}")
        raise
    except Exception as e:
        print(f"Lỗi không xác định khi ghi vào tệp tin {file_path}: {e}")
        return False
...
def read_file_content(file_path, default=""):
    ...
    Args:
        file_path (str): Đường dẫn đến tệp tin
        default (str): Giá trị mặc định nếu không đọc được
        
    Returns:
        str: Nội dung của tệp tin hoặc giá trị mặc định
    ...
    try:
        ...
        print(f"Lỗi I/O khi đọc tệp tin {file_path}: {e}")
        return default
    except Exception as e:
        print(f"Lỗi không xác định khi đọc tệp tin {file_path}: {e}")
        return default
...
def update_balance_file(file_path, profit_pct, original_balance):
    ...
    Args:
        file_path (str): Đường dẫn đến tệp tin số dư
        profit_pct (float): Phần trăm lợi nhuận
        original_balance (float): Số dư ban đầu
        
    Returns:
        float: Số dư mới sau khi cập nhật
    ...
    try:
        ...
        print(f"Lỗi I/O khi cập nhật tệp tin số dư {file_path}: {e}")
        raise
    except Exception as e:
        print(f"Lỗi không xác định khi cập nhật tệp tin số dư {file_path}: {e}")
